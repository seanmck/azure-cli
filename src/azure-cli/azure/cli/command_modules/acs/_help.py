...
examples:
  ...
  - name: Create a kubernetes cluster with supporting Windows agent pools.
    text: az aks create -g MyResourceGroup -n MyManagedCluster --load-balancer-sku Standard --vm-set-type VirtualMachineScaleSets --network-plugin azure --windows-admin-username azure --windows-admin-password 'replacePassword1234$'
